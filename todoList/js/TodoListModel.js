...
class TodoListModel {
    ...
    goItem() {
        ...
        // SHOW THE TOOLBAR AND LIST ITEM
    ...
    /**
    * This method creates a new list item and sets it up so that it
    * can be edited.
    */
    loadNewListItem() {
        this.listItemToAdd = new TodoListItem();
        this.prependList(this.listItemToAdd);
        this.view.loadListItemData(this.listItemToAdd);
    }
    ...
    loadItem(itemDescription) {
        ...
        for (let i = 0; i < this.listToEdit.getLength(); i++) {
            let testItem = this.listToEdit.getItemAtIndex(i);
            if (testItem.getDescription() === itemDescription) {
                itemToLoad = testItem;
                this.itemToEditPosition = i;
                i = this.listToEdit.getLength();
                this.itemToEdit = testItem;
            }
        }

        if (itemToLoad != null) {
            this.view.loadItemData(itemToLoad);
        }
    }
    ...
    createNewItem() {
        ...
        window.todo.model.listToEdit.addItem(this.itemToEdit);
